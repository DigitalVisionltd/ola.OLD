...
<?php //kyr hide subcategories in search field function use it in header.php
function cust_parent_categories_select($name = 'sCategory', $category = null, $default_str = null) {
    if($default_str == null) $default_str = __('Select a category');

    CategoryForm::category_select(Category::newInstance()->findRootCategoriesEnabled(), $category, $default_str, $name);
}
?>
...
function reklama($i) {
    ...
    // select from table
    $item = $conn->osc_dbFetchResults("SELECT * FROM kr_t_banner WHERE k_cat_id = '%d'", $i);
